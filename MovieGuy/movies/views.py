...
from datetime import datetime, timedelta
from .models import Movie, City, Theatre, Hall, Show
import random
import string
import vonage
    ...
    if query == 'all':
        ...
        if len(results) == 0:
            ...
            for movie in results:
                movie.image_url = movie.image.url
...
def card_detail(request, id):
    ...
    return render(request, 'movies/card_detail.html', context)
...
def verify_otp(request):
    if request.method == 'POST':
        entered_otp = request.POST.get('otp')  # Get the entered OTP from the POST data
        ...
        if entered_otp == '0359':  
            ...
            return HttpResponse("Invalid OTP, Go Back")
